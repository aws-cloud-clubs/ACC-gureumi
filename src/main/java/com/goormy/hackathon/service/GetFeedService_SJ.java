...
public class GetFeedService_SJ {
    ...
    @Transactional
    public List<GetFeedResponseDto_SJ> getFeedList(Long userId, int size) {
        ...
        if (postIdList.size() == size) {
            return getPostList(postIdList).stream().map((post) ->
                    GetFeedResponseDto_SJ.toDto(post, makePhotoUrl(post)))
                .toList();
        } else {
        ...
        List<Long> postIdListForReturn = splitPostSimpleInfoList(userRedisDS, sortedPushPullFeedList,
            size);
        // 5. post를 조회해서 반환
        return getPostList(postIdListForReturn).stream().map((post) ->
                GetFeedResponseDto_SJ.toDto(post, makePhotoUrl(post))).toList();

    }

    // post의 url 만들기
    private String makePhotoUrl(PostRedis_DS post) {
        return photoServiceSJ.getCDNUrl(post.getId(), post.getImgUrl());
    }
}
